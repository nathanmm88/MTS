...
<div id="sidebar-content">
    ...
    <table class="table table_summary">
                    ...
                    <?php if ($this->Entity->get('Order')->itemHasCondiments($orderItemKey)) { ?>
                        <?php foreach ($this->Entity->get('Order')->getCondimentsForItemId($orderItemKey) as $condimentInfo) { ?>
                            ...
                            <?php if ($condiment instanceof \App\Entity\Menu\CondimentEntity) { ?>
                                <p class="note"><b><?php echo $condimentType->getDescription() ?></b>
                                    - <?php echo $condiment->getDescription(); ?></p>
                            <?php } ?>
                        <?php } ?>
                    <?php } ?>
        ...
        <?php if ($this->Entity->get('Order')->isDelivery()) { ?>
            <tr>
                <td>Delivery</td>
                <td><strong
                        class="pull-right"><?php echo $this->Takeaway->formatMoney($this->Entity->get('Order')->getAddress()->getDeliveryCost()); ?></strong>
                </td>
            </tr>
        <?php } ?>
        <tr>
    ...
    <?php if ($this->Entity->get('Order')->isDelivery()) { ?>
        <p class="alert alert-info text-center">Delivering to
            <strong><?php echo $this->Entity->get('Order')->getAddress()->getPostcode(); ?></strong> at approximately
            <strong><?php echo $this->Entity->get('Order')->getEstimatedTime('g:ia'); ?></strong></p>
        <?php if ($this->Entity->get('Takeaway')->getSettings()->getAcceptCollectionOrders()) { ?>
        <?php } ?>
    <?php } else { ?>
        <p class="alert alert-info text-center">Collection from <br/><?php echo $this->Takeaway->getAddress('<br/>') ?>
            <br/> at approximately <strong><?php echo $this->Entity->get('Order')->getEstimatedTime('g:ia'); ?></strong>
        </p>
    <?php } ?>
</div>